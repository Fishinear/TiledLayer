...
typedef enum {
    ...
    TiledLayerAll = 0x04,
    TiledLayerLayoutNeeded = 0x4000, // internal use only
    TiledLayerLayoutOngoing = 0x8000 // internal use only
} TiledLayerFlags;
...
// of the provider; see the TileProvider interface for details.
...
//
// Note that you do not need to call layoutTilesWithFlags: if the existing tiles only need to be redrawn. Call setNeedsDisplay
...
// This method does not request new tiles. Call layoutTilesWithFlags: to refill the TileLayer with tiles.
// 
- (void) removeAll;
